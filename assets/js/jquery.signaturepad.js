...
(function ($) {
  function SignaturePad(selector, options) {
    ...
    function drawLine(e, newYOffset) {
      ...
      if (settings.drawBezierCurves === true) {
        ...
        if (strokePoints.length >= maxCacheLength) {
          ...
          var retrace = output.slice(
            output.length - maxCacheLength + 2,
            output.length
          );
    ...
    function typeIt() {
      ...
      typeItCurrentFontSize = typeItDefaultFontSize = $(settings.typed, context)
        .css('font-size')
        .replace(/px/, '');
    }
    ...
    function onFormError(errors, context, settings) {
      if (errors.nameInvalid) {
        ...
        $('label[for=' + $(settings.name).attr('id') + ']', context).addClass(
          settings.errorClass
        );
      }
    ...
    function strokePath(paths, context) {
      ...
      */
      ...
      for (var i = 0; i < paths.length - 1; i++) {
        ...
        if (typeof paths[i] === 'object' && typeof paths[i + 1] === 'object') {
          ...
          if (
            !(source.lx == destination.mx && source.ly == destination.my) &&
      ...
      */
      ...
      for (k = 0; k < sections.length; k++) {
        ...
        sections[k] = sections[k].filter(function (element, index) {
          return index % settings.bezierSkip == 0;
        });
        ...
        for (j = 0; j < section.length; j++) {
          ...
          var length =
            Math.abs(point.lx - point.mx) + Math.abs(point.ly - point.my);
          lengths.push(length);
      ...
      for (k = 0; k < sections.length; k++) {
        var section = sections[k];
        var simpleTuples = section.map(function (n) {
          return [n.lx, n.ly];
        });
        ...
        for (var i in beziers) {
          ...
          if (settings.variableStrokeWidth === true) {
            ...
            var zscore =
              (bezierSegmentLength - signatureStats.mean) /
              signatureStats.deviation;
            if (zscore > 0) {
              // fast
    ...
    function init() {
      ...
      if (!element.getContext && FlashCanvas) FlashCanvas.initElement(element);
